      ...
      for (var i = 0; i < acts.length; i++) {
        ...
        if (a.rail) {
          ...
          if (over > 0) {
            ...
            // In RTL the rail starts from the right edge, so its travel must be
            // positive to reveal later items toward the left. Re-read direction
            // here because language can change without remounting the runtime.
            var panDir = getComputedStyle(a.rail).direction === 'rtl' ? 1 : -1;
            a.rail.style.transform = 'translate3d(' + (panDir * (over + extra) * a.p).toFixed(2) + 'px,0,0)';
          }
        }
        ...
        for (var rv = 0; rv < a.reveals.length; rv++) {
          ...
          R.el.style.setProperty('--sc-reveal-progress', t.toFixed(4));
          R.el.style.clipPath =
